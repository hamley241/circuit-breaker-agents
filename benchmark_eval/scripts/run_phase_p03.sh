...
main() {
  ...
  echo "[INFO] execution_mode=state_locked pipeline_variant=three_stage fault_type=semantic inject_rate=0.3"

  ensure_prefix_is_safe "${prefix}" "${repeats}" "${policies[@]}"
  run_all_evals "${prefix}" "${input}" "${repeats}" "${max_parallel}" "${policies[@]}"
  summarize_all_runs "${prefix}" "${repeats}" "${policies[@]}"
  aggregate_all "${prefix}" "${repeats}" "${policies[@]}"
  summarize_aggregates "${prefix}" "${policies[@]}"

  echo "[DONE] Completed benchmark slice for prefix=${prefix}"
}
